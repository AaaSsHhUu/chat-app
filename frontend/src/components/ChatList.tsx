...
interface ChatItem {
    ...
    profileImg: string;
    lastMessage: string;
    lastReadTime: string | Date;
    unreadCount: number;
}

// const dummyChatList: ChatItem[] = [
//     {
//         roomId: "abc123",
...
function ChatList() {
    ...
    useEffect(() => {
        const fetchChatList = async () => {
            ...
            const res = await axios.get(`${import.meta.env.VITE_SERVER_URL}/user/rooms`, {
                headers : {
                    "Authorization" : `Bearer ${token}`
                }
            });
            console.log("rooms : ", res.data);
            setChatList(res.data?.rooms);
        }
        ...
        <div className="w-full bg-white dark:bg-slate-900 sm:w-full md:w-[30%] lg:w-80 h-full">
            ...
            <div className="flex flex-col w-full">
                {filterChatList.map((chat) => (
                    <Link
                        to={`/chats/${chat.roomId}`}
                        key={chat.roomId}
                        className={`flex items-center gap-3 px-4 py-3 cursor-pointer transition ${roomId === chat.roomId ? "bg-gray-300 dark:bg-slate-900" : "hover:bg-gray-200  dark:hover:bg-slate-800"}`}
                    >
                        ...
                        <div className="flex flex-col items-end">
                            <span className="text-xs text-gray-400">{chat.lastReadTime?.toString()}</span>
                            {
                                chat.unreadCount > 0 && (
